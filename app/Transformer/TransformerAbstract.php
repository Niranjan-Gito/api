...
use GitoAPI\Exceptions\CommonException;
...
abstract class TransformerAbstract extends FractalTransformer
{
    ...
    public function get($key = null)
    {
        ...
        if (! in_array($key, $allowedKeys)) {
            throw new CommonException(sprintf('Invalid key: "%s". Valid key(s): "%s"', $key, implode(', ', $allowedKeys)));
        }
    ...
    protected function validateParams()
    {
        ...
        if ($invalidParams = array_diff($usedParams, $validParams)) {
            throw new CommonException(sprintf('Invalid param(s): "%s". Valid param(s): "%s"', implode(', ', $usedParams), implode(', ', $validParams)));
        }
        return true;
    }
    /**
